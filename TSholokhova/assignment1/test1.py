...
class TestAnagrams(unittest.TestCase):
    ...
    def test_time1(self):
        ...
        for i in range(50000):
            self.assertTrue(is_anagram(s, s[::-1]))
        time_nlogn = time() - t0

        t0 = time()
        for i in range(50000):
            self.assertTrue(is_anagram_linear(s, s[::-1]))
        time_linear = time() - t0
        print(time_linear, time_nlogn)
        self.assertLess(time_linear, time_nlogn)
    ...
    def test_time2(self):
        ...
        s = 'abcdefghijklmnopqrstuvwxyz'
        t0 = time()
        self.assertTrue(is_anagram(s*500000, s[::-1]*500000))
        time_nlogn = time() - t0

        t0 = time()
        self.assertTrue(is_anagram_linear(s*500000, s[::-1]*500000))
        time_linear = time() - t0
        print(time_linear, time_nlogn)
        self.assertLess(time_linear, time_nlogn)
